...
library(tidyverse);library(rvest);library(magrittr);library(XML);library(janitor);library(stringr);library(rlang);library(readxl);library(readr)

#player id map
id_map <- read.csv('https://www.smartfantasybaseball.com/PLAYERIDMAPCSV', stringsAsFactors = F) %>% 
  clean_names() %>% 
  select(fangraphsname, pos)
pos_fg <- id_map %>% select(fangraphsname, pos) %>% 
  set_colnames(c('name','pos')) %>% 
  mutate(pos = str_extract(pos,"[^/]+")) %>% 
  filter(!is.na(pos))

#load subtraction vals for SGPs
hit_subt <- readr::read_csv('https://raw.githubusercontent.com/cuddjim/mlb/master/hit_sub.csv')
pit_subt <- readr::read_csv('https://raw.githubusercontent.com/cuddjim/mlb/master/pit_sub.csv')
...
proj_hit %<>% clean_proj() %>% 
  left_join(pos_fg, by = 'name') %>% 
  drop_na(pos) %>% filter(pos != 'P') %>% 
  distinct_player('pos') %>% 
  select(1:2, pos, everything())
proj_pit %<>% 
  clean_proj() %>% 
  mutate(pos = ifelse(gs/g < 0.5, 'RP','SP')) %>% 
  unique() %>% 
  select(1:2, pos, everything())
